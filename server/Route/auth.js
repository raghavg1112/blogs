...
router.post('/login', async (req, res) => {
    ...
    if (!find) {
        return res.status(201).json({ success: false, message: "Username doesn't exist's" });
    }
    ...
    return res.status(201).json({ success: true, message: "Login Successfull", authToken: authToken, user_id: find.email, name: find.name });
})
...
router.post('/signUp', async (req, res) => {
    ...
    if (!name || !email || !password || !c_password) {
        return res.status(201).json({ success: false, message: "fill all credentials properly" });
    }
    if (password != c_password) {

        return res.status(201).json({ success: false, message: "password and confirm password dont match", })
    }
    ...
    if (found) {
        ...
        const user = new User({ name: name, email: email, password: secure_pass, c_password: secure_c_pass })
        user.save().then(() => {
            return res.status(201).json({ success: true, message: "registered successfull" })
        })
    }
...
router.get('/my_old_blogs', async (req, res) => {
    ...
    console.log(blogs)
    if(!blogs){
        res.status(200).json({ success:false,message:"Failed to load Your Old Blogs" });
    }
...
router.get('/get_all_blogs',async(req,res)=>{
    ...
    if(!blogs){
        return res.status(200).json({success:false,message:"failed to load Public blogs!! Please RELOAD!!"});
    }
...
router.post('/create_blog', async (req, res) => {
    ...
    const create_blog=new Blog({user_id:user_id,blog:blog})
  const check_save=await create_blog.save();
  if(!check_save){
    return res.status(200).json({success:false,message:"Unable to Create Blog"})
  }
...
if(!check){
    return  res.status(200).json({success:false,message:"Unable to Delete"});
}
...
router.post('/edit_blog',async(req,res)=>{console.log(`edit`)
    const blog=req.body;
    console.log(req.body);
     Blog.findOneAndUpdate({_id:blog._id},{blog:blog.blog},(err,data)=>{
